...
3. **Low-code configuration surface**: an admin screen where a program owner edits a question label, help text, or FAQ entry, adds a new question point-and-click, sets the answer that makes it appear, changes when existing questions appear, and moves a question to another section, with per-field revert; every rule change and relocation is checked against the generated decision matrix and refused if it would strand a question; edits land in a draft and reach reporters only on an explicit publish, which is recorded with its author and description
