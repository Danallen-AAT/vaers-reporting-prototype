...
3. **Low-code configuration surface**: an admin screen where a program owner edits a question label, help text, or FAQ entry, adds a new question point-and-click, sets the answer that makes it appear, and changes when existing questions appear, with per-field revert; the live form re-renders from configuration
